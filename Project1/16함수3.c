...
// 알수 있는 데이터를 전달해줍니다.
//.4 va_list는 전달 받은 가변인자를 저장할 데이터입니다.
//5. va_start(args,count)는 가변인자에 대한 처리를 진행합니다.
//6. va_arg(args,자료형)은 args로부터 하나의 값을 가져오고 그 값을 자료형으로 명시합니다.
//7. va_end(args) : 가변인자에 대한 처리를 종료하고 기능을 제거합니다.(제거안하면 프로그램에 데이터 남음 필수로 제거)
...
void printNumbers(int count, ...)
{
	...
	//전달받은 값을 묶음으로 저장합니다.
	va_start(args, count);
	//목록으로 부터,count까지의 작업을 수행합니다.

	for (int i = 0; i < count; i++)
	{
		printf("%d", va_arg(args, int));
		//va_arg(묶음 이름,자료형)을 통해
		//묶음에 있는 자료형 데이터의 값을 받아옵니다.
	}
	va_end(args);//작업 완료
}

//c언어에서 자주 사용되는 인자를 알아봅시다.
// 1.문자열을 받는 경우
...
//2. 배열을 받는 경우
//배열을 전달할 경우에는 c언어가 자동적으로 배열의 길이를 확인할 수 없기 때문데 따로 배열의 길이는 작성해 줘야 합니다.
...
int main()
{
	printNumbers(5, 10, 20, 30, 40, 50,60);

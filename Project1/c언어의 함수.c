...
//2. 반환 타입에 따라 함수에서는 return을 통해 전달해줘야 합니다.
//3. 매개변수는 함수가 필요한 입력 값을 받기 위해 정의해주는 변수입니다. 외부로 부터 전달받은 값을 함수 내부의 값으로써 사용하게 해주며
...
int sub(int a, int b)
{
	return a - b;//긴단한 계산 경과라면 return을 통해 한번에 작성하는게 위의 방식보다 읽기가 편합니다
}
...
int main()
{
	int c = add(2, 7);
	//1. add(2,7)ghcnf
	//2. 매개변수에 인자 값 전달 a=2b=7
	// 인자 값(Argument)는 매개변수처럼 함수에서 값을 전달하는 과정과 관련된 용어입니다.
	// 매개변수가 외부에서 받을 값에 대한 표현이라면, 인자의 경우는 함수 호출 시 실제로 전달되는 값
	// 
	//3. 내부의 식 실행 result =2+7;
	//4. return에 의해 함수 종료,return다음에 적힌 값을 함수의 호출 위치에 전달
	//5. int c = 5;

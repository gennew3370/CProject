...
int main()
{
	...
	int a = 7;
	setAvalue(a, 10);
	//1. 인자값 (a : 7), 10을 setAvalue에 전달합니다.
	//2. 매개변수 a에 인자값(a)의 값을 적용합니다.value에는 10을 적용합ㄴ디ㅏ.
	//3. a에 value의 값을 적용합니다.
	//4. 함수가 종료되면 매개변수 a의 값은 삭제됩니다.

	printf("a = %d\n", a);
	//바꾼건 안자값(a)거 아닌 함수 내부의 값(매개변수) a 이기 때문에 변화가 없습니다.

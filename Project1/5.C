...
int main()
{
    ...
    //혼합 대입 연산자
    // = 전에 +<-,*,%를 작성하는 경으 대입하여는 값에 연산을 진행한 결과를 대입합니다.
    //ex) 위에 코드에서 추가적으로 다음과 같이 명령을 작성합니다,
    //a += 5;라고 적으면 a에 5를 더한 결과를 a에 대립합니다. 따라서 10이 됩니다.
    //즉 a += 5는 a = a + 5와 같ㅇ,ㄴ 말입니다.
    ...
    //b = a++일 경우를 후위형 증강 연산이라고 부르며,이 경우에는 연산을 먼저 진행한 뒤, a의 값을 1증가시킵니다.
    //따라서 대입연산에 의해 b는 a(10)의 값을 대입하게 되고,그 후에 a가 1증가합니다. 

    //b = ++a일 경우를 전위형 증강 연산이라고 부르며, 이 경우에는 a에 대한 1증가를 먼저 진행하고, 연산을 처리합니다.
    ...
    //관계 연산자는 아래의 if문 같이 조건에 따라 코드를 작성하는 상황애 사용됩니다.
    a = 10;
    b = 7;
